...
const BadgeButton = ({ children, selected, ...props }: BadgeButtonProps) => {
  ...
  return (
    <button className={`flex h-[30px] items-center rounded-md border px-4 py-2 text-xs ${buttonStyle}`} {...props}>
      {children}
    </button>
  );
};
